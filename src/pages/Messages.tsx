...
const Messages = () => {
  ...
  // Create or select existing conversation with another user
  const createOrSelectConversation = useCallback(async (otherUserId: string) => {
    if (!user) return null;
  ...
  useEffect(() => {
    ...
    fetchConversations();
  }, [user, authLoading, targetUserId, createOrSelectConversation]);
